...
## by Steve Myles
##
## GitHub URL:  https://github.com/scumdogsteev/datasciencecoursera
## repo:  {GitHub URL}/tree/master/getting-and-cleaning-data
## README:  {GitHub URL}/blob/master/getting-and-cleaning-data/README.md
## CodeBook:  {GitHub URL}/blob/master/getting-and-cleaning-data/CodeBook.md
##
## This is run_analysis.R.  It produces a wide-form tidy data set by merging 
## the "train" and "test" data and returning only the measurements of mean and
## standard deviation.  Comments describe the steps.  Comments in ALL CAPS and
## surrounded by asterisks are the processing steps and those in lowercase
## represent the detail required to complete the major steps.
##
## Please also see the accompanying README and CodeBook in the GitHub repo
## above for further detail on the logic.

# to do:
# - add error checking - exit if files don't exist
...
## read "activity_labels.txt" into a data frame and change its column names to
## "ActKey" and "Activity"
...
## convert the "Feature" column of the features data frame (features$Feature)
## to character
...
## read the test activities file (y_test.txt) and name the column "ActKey"
...
## add the activity labels by performing a left join (using the "left_join"
## function from the dplyr package)
merged <- left_join(activity_labels, merged, by = "ActKey")

## change the names of the columns in the "data" data frame to "ActKey," 
## "Activity," and "Subject" for the first three columns and the values in the
## "Feature" column of the features data frame for the remaining columns
...
## "Activity," and any of the remaining columns with "Mean," "mean," or "std"
## in their name
...
## remove the parentheses from the column names to make them legal for R by
## substituting a blank ("") for all instances of "()" (using escape 
##characters) and "-" 
...
## sort the merged data frame by Subject then Activity using the "arrange"
## function from the plyr package
merged <- arrange(merged, Subject, Activity)

## ***************************************************************************
## CALCULATE THE MEAN FOR EACH MEASUREMENT VARIABLE
## ***************************************************************************

## using the "melt" function from the reshape2 package, melt the merged data
## frame to create a long-form data set
merged_melt <- melt(merged, id.vars=c("Subject","Activity"),
                    variable.name = "Measurement", value.name = "Value")

## create a summary data frame with the average (mean) of each variable for
## each Subject/Activity combination using the dcast function from the 
## "reshape2" package.  this is a wide-form tidy data frame.
merged_summary <- dcast(merged_melt, Subject + Activity~Measurement, mean,
                        value.var = "Value")
